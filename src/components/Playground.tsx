...
const Playground: React.FC<PlaygroundProps> = ({ providers }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-slate-700 bg-slate-700 flex items-center justify-between">
        <h2 className="text-xl font-semibold">AI 演练场</h2>
        <select
          value={selectedProvider}
          onChange={(e) => setSelectedProvider(e.target.value)}
          className="px-3 py-2 bg-slate-600 border border-slate-500 rounded-md text-white text-sm"
        >
